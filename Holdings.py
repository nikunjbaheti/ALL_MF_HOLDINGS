...
def filter_latest_data(df, date_col='invdate'):
    ...
    # Split the datetime string using "T" as a delimiter, keeping only the date part, and convert it to date
    df[date_col] = pd.to_datetime(df[date_col].str.split("T").str[0], errors='coerce').dt.date
...
for scheme_code in unsuccessful_codes:
    ...
    # print(f"Unsuccessful {index +1} scheme codes")
    print(f"Successfully processed {successful_count}/{len(schemecodes)} scheme codes")
